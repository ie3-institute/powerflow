...
object LibraryLoader {
  ...
  /** Method for loading a native library.
    ...
    * @param default
    *   A fallback implementation using no native code.
    * @tparam T
    *   Type of loaded library.
    * @return
    *   Either the loaded native library or the default as a fallback.
    */
  def load[T](
  ...
  /** Method for loading a native library.
    ...
    *   Either the loaded native library or the default as a fallback.
    */
  def load[T](path: String, native: String => T, default: T): T = try {
    native(path)
  } catch {
    case e: Throwable =>
      log.warn(
