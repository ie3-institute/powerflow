...
class ThreeNodesJacobianMatrixSpec extends UnitSpec with ThreeNodesTestData {
  val testTolerance = 1e-3
  "Three node grid - The jacobian matrix" should {
    "calculate the correct jacobian matrix" in {
      ...
      do {
        if abs(
            actual(rowIdx, colIdx) - expectedJacobianMatrix(rowIdx, colIdx)
          ) > testTolerance
        then
          logger.debug(
            s"Mismatch in ($rowIdx, $colIdx): Actual = ${actual(rowIdx, colIdx)}, expected = ${expectedJacobianMatrix(rowIdx, colIdx)}"
          )
      }
